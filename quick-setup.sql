-- Quick setup for license management
...
INSERT INTO license_plans (name, slug, description, pricing_model, price_per_unit, billing_cycle, included_apps, features) VALUES
  ('Align Professional', 'align-pro', 'Professional goal alignment and OKR management', 'per_user', 15.00, 'monthly', ARRAY['align'], '{"goals_per_user": 50, "objectives_per_org": 20}'),
  ('Startup Trial', 'startup-trial', '14-day free trial of the full suite', 'organization', 0.00, 'one_time', ARRAY['align', 'drive'], '{"trial_days": 14, "max_users": 10}')
...
-- 6. Make current user super admin (this will work if you're signed in)
INSERT INTO users (
  id,
  email, 
  full_name, 
...
SELECT 
  auth.uid(),
  COALESCE((auth.jwt() ->> 'email'), 'admin@example.com'),
  COALESCE((auth.jwt() -> 'user_metadata' ->> 'full_name'), 'Super Admin'),
  'super_admin',
  NOW(),
  NOW()
...
SELECT 
  'License Plans' as table_name,
  COUNT(*) as count
FROM license_plans
UNION ALL
SELECT 
  'Current User',
  CASE WHEN is_super_admin() THEN 1 ELSE 0 END
UNION ALL  
SELECT
  'Organizations',
  COUNT(*)
